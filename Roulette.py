...
def place_bet():
    ...
    bet_options=[
        ...
        ("Low (1-18)",6),
        ("High (19-36)",7),
        ("Column (1st, 2nd, 3rd)",8),
        ("Dozen (1-12, 13-24, 25-36)",9),
    ]
    ...
    def confirm_bet():
        ...
        elif bet_type in [8,9]:
            bet_choice=tk.simpledialog.askstring("Input","Enter your choice (1, 2, 3):")
            if bet_choice not in ["1","2","3"]:
                messagebox.showerror("Error","Invalid choice. Please select a valid option.")
                return
            spin_roulette(bet_choice,bet_amount,bet_type)
        else:
...
def spin_roulette(bet,bet_amount,bet_type):
    ...
    if bet_type==5:
        ...
        if bet_type==1 and result in red_numbers():
            ...
            reason=f"you bet on {bet_type}."

    if win:
        messagebox.showinfo("Result",f"You won! Payout: ₹{payout} because {reason}")
        balance+=payout
    else:
        messagebox.showinfo("Result",f"You lost because {reason}")
        balance-=bet_amount
